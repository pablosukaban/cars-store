  ...
  return (
    <div className="mt-10 grid h-[86vh] place-items-center overflow-hidden bg-secondaryLightGray px-4 md:mt-0">
      <div className="rounded border border-secondaryLightGray bg-white px-8 py-6 shadow-lg sm:px-16 sm:py-12">
        <h1 className="mb-4 text-center text-2xl font-bold">{isLoginPage ? "Авторизация" : "Регистрация"}</h1>
        ...
        <div className=" flex flex-wrap items-start justify-between gap-8">
          {isLoginPage ? (
            <div className="">
              Нет аккаунта?{" "}
              <NavLink to={REGISTRATION_ROUTE} className={"text-primaryOrange transition hover:underline"}>
                Зарегистрироваться
              </NavLink>
            </div>
          ) : (
            <div className="">
              Есть аккаунт?{" "}
              <NavLink to={LOGIN_ROUTE} className={"text-primaryOrange transition hover:underline"}>
                Войти
              </NavLink>
            </div>
          )}
